...
{
	[CustomEditor(typeof(RuleTile2))]
	[CanEditMultipleObjects]
	internal class RuleTile2Editor : Editor
	{
		...
		public RuleTile2 tile { get { return (target as RuleTile2); } }
		...
		public void OnEnable()
		{
			if (tile.m_TilingRules == null)
				tile.m_TilingRules = new List<RuleTile2.TilingRule>();

			m_ReorderableList = new ReorderableList(tile.m_TilingRules, typeof(RuleTile2.TilingRule), true, true, true, true);
		...
		{
			RuleTile2.TilingRule rule = tile.m_TilingRules[index];
		...
		public override void OnInspectorGUI()
		{
			...
			tile.m_NeighborType = (RuleTile2.NeighborType)EditorGUILayout.EnumPopup("Check Type", tile.m_NeighborType);
		...
		private static void RuleMatrixOnGUI(Rect rect, RuleTile2.TilingRule tilingRule)
		{
			...
			for (int y = 0; y <= 2; y++)
			{
				for (int x = 0; x <= 2; x++)
				{
					...
					if (x != 1 || y != 1)
					{
						...
						{
							case RuleTile2.TilingRule.Neighbor.This:
								GUI.DrawTexture(r, arrows[y*3 + x]);
								break;
							case RuleTile2.TilingRule.Neighbor.NotThis:
								GUI.DrawTexture(r, arrows[9]);
								break;
						}
						if (Event.current.type == EventType.MouseDown && r.Contains(Event.current.mousePosition))
						{
							tilingRule.m_Neighbors[index] = (RuleTile2.TilingRule.Neighbor) (((int)tilingRule.m_Neighbors[index] + 1) % 3);
							GUI.changed = true;
							Event.current.Use();
						}
					...
					else
					{
						...
						{
							case RuleTile2.TilingRule.Transform.Rotated:
								GUI.DrawTexture(r, autoTransforms[0]);
								break;
							case RuleTile2.TilingRule.Transform.MirrorX:
								GUI.DrawTexture(r, autoTransforms[1]);
								break;
							case RuleTile2.TilingRule.Transform.MirrorY:
						...
						if (Event.current.type == EventType.MouseDown && r.Contains(Event.current.mousePosition))
						{
							tilingRule.m_RuleTransform = (RuleTile2.TilingRule.Transform)(((int)tilingRule.m_RuleTransform + 1) % 4);
							GUI.changed = true;
							Event.current.Use();
						}
					}
		...
		{
			public RuleTile2.TilingRule m_Rule;
			public RuleTile2.TilingRule.Transform m_NewValue;

			public MenuItemData(RuleTile2.TilingRule mRule, RuleTile2.TilingRule.Transform mNewValue)
			{
				this.m_Rule = mRule;
		...
		private void SpriteOnGUI(Rect rect, RuleTile2.TilingRule tilingRule)
		{
		...
		private static void RuleInspectorOnGUI(Rect rect, RuleTile2.TilingRule tilingRule)
		{
			float y = rect.yMin;
			EditorGUI.BeginChangeCheck();
			GUI.Label(new Rect(rect.xMin, y, k_LabelWidth, k_SingleLineHeight), "Rule");
			tilingRule.m_RuleTransform = (RuleTile2.TilingRule.Transform)EditorGUI.EnumPopup(new Rect(rect.xMin + k_LabelWidth, y, rect.width - k_LabelWidth, k_SingleLineHeight), tilingRule.m_RuleTransform);
			...
			tilingRule.m_Output = (RuleTile2.TilingRule.OutputSprite)EditorGUI.EnumPopup(new Rect(rect.xMin + k_LabelWidth, y, rect.width - k_LabelWidth, k_SingleLineHeight), tilingRule.m_Output);
			y += k_SingleLineHeight;

			if (tilingRule.m_Output == RuleTile2.TilingRule.OutputSprite.Animation)
			{
			...
			if (tilingRule.m_Output == RuleTile2.TilingRule.OutputSprite.Random)
			{
				...
				tilingRule.m_RandomTransform = (RuleTile2.TilingRule.Transform)EditorGUI.EnumPopup(new Rect(rect.xMin + k_LabelWidth, y, rect.width - k_LabelWidth, k_SingleLineHeight), tilingRule.m_RandomTransform);
				y += k_SingleLineHeight;
			}

			if (tilingRule.m_Output != RuleTile2.TilingRule.OutputSprite.Single)
			{
